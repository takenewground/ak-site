...
function __main__() {
    ...
    rig.man = man;
    man.msgbox.set($EVAL, function eval(msg) {
...
rig.hash32_str = murmurhash3_32_gc;
...
class SerDes {
    ...
    ser_str(str) {
        ...
        // if (len > SERDES_MAX_STR_LEN) throw new Error(`ser_str: len > SERDES_MAX_STR_LEN`)
        const {read, written} = utf8enc_to(str, this.bytes, this.i);
        if (read !== len) throw new Error(`ser_str: len:${len} exceeded target buffer size:${this.bytes.byteLength}`)
        this.set_u16(j, size);
        this.move(written);
    }
...
class Msg extends SerDes {
    ...
    get_flags()         {return this.set_u16(MSG_FLAGS_POS);}
...
function msg_set_flags(msg, flags) {
    const j = msg.jump(MSG_FLAGS_POS);
    msg.ser_i32(flags);
    msg.jump(j);
}

function msg_set_cb(msg, dstbox) {
    const j = msg.jump(MSG_SRC_BOX_POS);
    msg.ser_i32(dstbox);
    msg.jump(j);
}
...
function _dispatch_msg(msg) {
    const dst = msg.get_dst();
    if (!actor_has(dst)) throw new Error("_dispatch_msg: actor not found");
    const dst_actor = actor_get(dst);
    dst_actor.on_msg(msg);
}

function msg_send(src, msg, dst, dstbox) {
    const j = msg.jump(MSG_DST_POS);
    msg.ser_i32(dst);
    msg.ser_i32(dstbox);
    msg.ser_i32(src);
    ...
    if (dst_thid === rig.man.id)
        ...
        _msg_send_enqueue(_dst_to_thid(dst), msg)
}
...
function _msg_send_enqueue(dst_thid, msg) {
...
const THREAD_T      = 0b00000001;
const WORKER_T      = 0b00000101;
...
Actor.prototype.log = function (...args) {
    console.log(`Actor:${this.constructor.name}:${this.id}::`,...args);
}
Actor.prototype.warn = function (...args) {
    console.warn(`Actor:${this.constructor.name}:${this.id}::`,...args);
}
Actor.prototype.error = function (...args) {
    console.error(`Actor:${this.constructor.name}:${this.id}::`,...args);
}
